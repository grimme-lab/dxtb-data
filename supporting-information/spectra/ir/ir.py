...
f = Path(__file__).parent / f"{FILE}.xyz"
...
prefix = Path(__file__).parent / f"ir-{FILE}"


print("AutoDiff")

# AUTODIFF
res = calc.ir(numbers, positions, charge)
res.use_common_units()
res.save_prop_to_pt("freqs", f"{prefix}-freqs.pt")
res.save_prop_to_pt("ints", f"{prefix}-ints.pt")

calc.reset()
print("Numerical")

# NUMERICAL
res = calc.ir_numerical(numbers, positions, charge)
res.use_common_units()
res.save_prop_to_pt("freqs",  f"{prefix}-num-freqs.pt")
res.save_prop_to_pt("ints", f"{prefix}-num-ints.pt")
